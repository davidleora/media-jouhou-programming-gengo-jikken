...
void get_field_state_from_field_cards(int cards[8][15], state *field_status)
{
  ...
  {
    if (cards[i][j + 1] > 0)
      field_status->is_sequence = 1;
    else
      field_status->is_sequence = 0;
  }
  // 枚数を数える また強さを調べる
...
void get_field_state_from_own_cards(int cards[8][15], state *field_status)
{
  ...
  // 状態
  if (cards[5][4] > 0)
    field_status->is_no_card = 1; // 場にカードがないとき 1
  else
    field_status->is_no_card = 0;
  if (cards[5][6] > 0)
    field_status->is_rev = 1; // 革命状態の時 1
  else
    field_status->is_rev = 0;
  if (cards[5][7] > 0)
    field_status->is_lock = 1; // しばり時 1
  else
  ...
  for (i = 0; i < 5; i++)
    field_status->player_quantity[i] = cards[6][i]; // 手持ちのカード
  for (i = 0; i < 5; i++)
    field_status->player_rank[i] = cards[6][5 + i]; // 各プレーヤのランク
  for (i = 0; i < 5; i++)
    field_status->seat[i] = cards[6][10 + i]; // 誰がどのシートに座っているか
  // シートiにプレーヤ STATE.SEAT[I]が座っている

  if (cards[4][1] == 2)
    field_status->have_joker = 1; // Jokerがある時 1
  else
    field_status->have_joker = 0;
}
...
void remove_low_pair(int cards[8][15], int num, int rev)
{
  int i, j;
...
void remove_low_sequence(int cards[8][15], int num, int rev)
{
  int i, j;
